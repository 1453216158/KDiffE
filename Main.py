...
class Coach:
	...
	def trainEpoch(self):
		...
		diffusionLoader1 = self.handler.diffusionLoader1

		for i, batch in enumerate(diffusionLoader1):
			...
			diff_loss_image, gc_loss_image = self.diffusion_model.training_losses(self.denoise_model_image, batch_item,itmEmbeds=iEmbeds, batch_index=batch_index, model_feats=image_feats)
			diff_loss_text, gc_loss_text = self.diffusion_model.training_losses(self.denoise_model_text, batch_item,itmEmbeds=iEmbeds, batch_index=batch_index, model_feats=text_feats)
			if args.data == 'tiktok':
				diff_loss_audio, gc_loss_audio = self.diffusion_model.training_losses(self.denoise_model_audio, batch_item, itmEmbeds=iEmbeds, batch_index=batch_index, model_feats=audio_feats)
			loss_image = diff_loss_image.mean() + gc_loss_image.mean() * args.e_loss
			loss_text = diff_loss_text.mean() + gc_loss_text.mean() * (1 - args.e_loss)
			if args.data == 'tiktok':
		...
		for i, batch in enumerate(diffusionLoader):
			...
			loss = diff_loss.mean() + ukgc_loss.mean() * args.e_loss
			epDiLoss += loss.item()
		...
		log('')
		log('Start to re-build kg and UI matrix')
		...
		with torch.no_grad():
			...
			for _, batch in enumerate(diffusionLoader1):
				batch_item, batch_index = batch
				batch_item, batch_index = batch_item.cuda(), batch_index.cuda()

				uEmbeds = self.model.getUserEmbeds().detach()
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_image, batch_item,args.sampling_steps,batch_index,uEmbeds)
				top_item, indices_ = torch.topk(denoised_batch, k=args.q)
				...
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model_text, batch_item, args.sampling_steps,batch_index,uEmbeds)
				top_item, indices_ = torch.topk(denoised_batch, k=args.q)
				...
				if args.data == 'tiktok':
					...
					denoised_batch = self.diffusion_model.p_sample(self.denoise_model_audio, batch_item,args.sampling_steps,batch_index,uEmbeds)
					top_item, indices_ = torch.topk(denoised_batch, k=args.q)
			...
			for _, batch in enumerate(diffusionLoader):
				batch_item, batch_index = batch
				batch_item, batch_index = batch_item.cuda(), batch_index.cuda()
				denoised_batch = self.diffusion_model.p_sample(self.denoise_model, batch_item, args.sampling_steps,batch_index,uEmbeds)
				top_item, indices_ = torch.topk(denoised_batch, k=args.q)
				for i in range(batch_index.shape[0]):
					for j in range(indices_[i].shape[0]):
		...
		for i, tem in enumerate(trnLoader):
			...
			if args.cl_pattern == 0:
				...
				if args.data == 'tiktok':
					usrEmbeds0, itmEmbeds0 = self.model.forward_MM(self.handler.torchBiAdj, self.image_UI_matrix,self.text_UI_matrix, self.audio_UI_matrix)
				else:
					usrEmbeds0, itmEmbeds0 = self.model.forward_MM(self.handler.torchBiAdj, self.image_UI_matrix,self.text_UI_matrix)
			else:
			...
			ancEmbeds0 = usrEmbeds0[ancs]
			posEmbeds0 = itmEmbeds0[poss]
			negEmbeds0 = itmEmbeds0[negs]

			scoreDiff = pairPredict(ancEmbeds, posEmbeds, negEmbeds)+ pairPredict(ancEmbeds0, posEmbeds0, negEmbeds0)
			...
			clLoss0 = (contrastLoss(kgEmbeds[args.user:], denoisedKGEmbeds[args.user:], poss, args.temp) + contrastLoss(
			...
			if args.data == 'tiktok':
				...
				clLoss_ = clLoss1 + clLoss2 + clLoss3+clLoss0
			else:
				clLoss_ = clLoss1 + clLoss2+clLoss0
		...
		ret['BPRLoss'] = epRecLoss / steps
		ret['clLoss'] = epClLoss / steps
		if args.data == 'tiktok':
			ret['diLoss'] = (epDiLoss + epDiLoss_image + epDiLoss_text+epDiLoss_audio) / diffusionLoader.dataset.__len__()
		else:
			ret['diLoss'] = (epDiLoss + epDiLoss_image + epDiLoss_text ) / diffusionLoader.dataset.__len__()
		return ret
	...
	def testEpoch(self):
		...
		with torch.no_grad():
			if args.cl_pattern == 0:
				...
				usrEmbeds0, itmEmbeds0 = self.model(self.handler.torchBiAdj, mess_dropout=False, kg=denoisedKG)
			else:
				usrEmbeds0, itmEmbeds0 = self.model(self.handler.torchBiAdj, mess_dropout=False)
			if args.data == 'tiktok':
				usrEmbeds1, itmEmbeds1 = self.model.forward_MM(self.handler.torchBiAdj, self.image_UI_matrix,
		...
		for usr, trnMask in tstLoader:
			...
			allPreds = t.mm(usrEmbeds0[usr], t.transpose(itmEmbeds0, 1, 0)) * (1 - trnMask) - trnMask * 1e8+\
										t.mm(usrEmbeds1[usr], t.transpose(itmEmbeds1, 1, 0)) * (1 - trnMask) - trnMask * 1e8
